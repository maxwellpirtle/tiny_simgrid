                                  ...
                                  unsigned long sizeD, int inter)
{
  ...
  // if inter > 0 intS already has a presentative, pop the old one and add the
  // new one
...
EventSet UnfoldingChecker::KpartialAlt(EventSet D, Configuration C) const
{
  ...
  // building J by chosing one event in each spike such that there are not 2
  // conflict events in J
  auto n = 0;
...
bool transition_is_IReceive(EventSet const& testedEvtHist, EventSet const& ancestorsHist, EventSet const& ancestors,
                            const UnfoldingEvent* testedEvt)
{
  auto nbReceive             = 0;
...
bool transition_is_ISend(EventSet const& testedEvtHist, EventSet const& ancestorsHist, EventSet const& ancestors,
                         const UnfoldingEvent* testedEvt)
{
  auto nbSend                = 0;
  auto testedEvt_trans_mb_id = App::app_side_->get_transition_mb_id(testedEvt->get_transition_tag());
...
bool checkSdRcCreation(std::string const& trans_tag, EventSet ancestors, Configuration C)
{
  ...
  // check nb send == nb receive ?, if yes they concern the same comm -> test
  // and send/receive are dependent
...
/* this function creating new events from a transition and a set cadidate
 * directed ancestors (ancestorSet) (the set includes events that can be a
 * direct ancestor) by combination the transition and all subset (cause)  of the
 * set ancestorSet
 */
bool check_contains(bool chk, EventSet const& cause, const UnfoldingEvent* immPreEvt)
{
  bool chk1 = false;
  // if immediate precede event is not in the causuality_events, ensure that it
  // is in the history of one event in cause
  if (!chk) {
    for (auto evt : cause) {
...
void Configuration::createEvts(Configuration C, EventSet& result, const std::string& trans_tag, s_evset_in_t ev_sets,
                               bool chk, UnfoldingEvent* immPreEvt)
{
  ...
  if (ancestorSet.empty()) {
     ...
     * if all conditions are passed (trans is enabled, any ancestors are not in
     * the history of other ancestors)
     */
    if (chk || chk1) {
      bool send_receiveCheck = true;
      EventSet cause1;
      cause1 = EvtSetTools::makeUnion(cause, causuality_events);
      // check condition for send/receive action or check enabled for MutexWait
      // action
...
void create_events_from_trans_and_maxEvent(std::tuple<Configuration, bool, std::string const&> t_params,
                                           EventSet const& causalityEvts, UnfoldingEvent* immPreEvt,
                                           EventSet const& ancestorSet, std::list<EventSet>& maxEvtHistory,
                                           EventSet& exC)
{
  ...
  /*2. We now compute new evts by using MaxEvts in the past, but we have to
   * ensure that the ancestor events (used to generate history candidate) are
   * not in history of the evts in the causalityEvts
   */
  std::vector<int> intS;
...
/* this function produces new events from a given transition (trans) and the
 * maxEvtHistory*/
EventSet computeExt(Configuration C, std::list<EventSet> maxEvtHistory, std::string const& trans_tag)
{
  ...
  // add causality evts to causalityEvts set, firstly add last event to
  // causalityEvts
  ...
  *  used to make sure trans is enabled (Ti is enlabled if Ti-1 is aready fined)
  chk == true =>  causalityEvts contains immediate precede event of trans Or the
  immediate precede event is in history of lastEvt.
  */
  ...
  if (trans_id == 0)
    chk = true; // if trans.id ==0 => trans is always enabled do not need to
  ...
  // 1.1 if only last evt and immidiate precede event are dependent with trans
  // -> only one evt is created
  if (causalityEvts.size() <= 2 && ancestorSet.size() == 0) {
    g_var::nb_events++;

    /* in this case only one event is created, since all MaxEvts are in the
     * history of lastEvt*/
...
this wait waits a communication (action send/receive) in the the parameter/event
evt The idea here is that, we try to march the communication with all possible
communication to crete a complete communication, making wait become enabled.
When the wait enable, we can create new events
*/
EventSet createWaitEvt(const UnfoldingEvent* evt, Configuration C, std::string const& trans_tag)
{
  ...
  if (comType == "Isend") {
    // if waited communication is  send, count the number of send request before
    // the communication
    for (auto evt1 : hist) {
    ...
    for (auto evt2 : C.events_) {
      ...
      if ((evt2_trans_type == "Ireceive") && (evt2_trans_mb_id == mbId)) {
        ...
        // count the number of receice requests before the receive that we found
        // above
        ...
        if (nbSdRc == nbRc) {
          // if the number send = number receive, we can march the send
          // communication  with the rececive
          EventSet ancestors;
...
EventSet createTestEvt(EventSet exC, UnfoldingEvent* evt, Configuration C, std::string const& trans_tag)
{
  ...
  /* since action test is always enabled, create a new event although there is
 no pending Ireceive
 ...
 * */
  ...
  *						   |-> pre evt is in the history of last
  evt *2. last evt != pre evt =>  |-> TRY TO create one evt
  ...
  */
  ...
  // maxEvt =  C.findActorMaxEvt(evt->transition.actor_id);
  {
    ...
    /* Now try to create newEvt2 if there is a pending communication can be
   march with the communication tested by the Test action */
    ...
    if (comType == "Isend") {
      ...
      for (auto evt2 : C.events_) {
        ...
        auto check_ireceive = (evt2_trans_type == "Ireceive") && (evt2_trans_mb_id == mbId) &&
        ...
        if (check_ireceive) {
          ...
          // count the number of receice requests before the receive that we
          // found above
    ...
    } else if (comType == "Ireceive") // do the same for a receive
    {
      ...
      for (auto evt2 : C.events_) {
        ...
        auto check_isend =
            (evt2_trans_type == "Isend") && (evt2_trans_mb_id == mbId) && (!EvtSetTools::contains(lastEvtHist, evt2));
        if (check_isend) {
          // after find out a receive request
          EventSet hist2 = evt2->getHistory();
          int nbSend     = 0;

          // count the number of receice requests heppen before the receive that
          // we found above
  ...
  } else // If the last event is evt2 (in figure), try to march evt2 with evt
         // (in figure)
  {
    if (comType == "Isend") {
      int nbReceive = 0;
...
EventSet createIsendEvts(std::string const& trans_tag, Configuration C)
{
  ...
  if (check0) {
    ...
    // two sends or two receives can not be in the same communication -> not
    // denpendent
    if (check1)
      return EvtSet;
  }

  // if trans.id ==0 => trans is always enabled do not need to check enable
  // condition ?.
  auto check2 = (trans_id == 0) || (lastEvt_trans_actor_id == trans_actor_id);
  if (check2)
    enableChk = true;
  /* enableChk = true -> trans is ensured enabled or his pre evt is in the
   * causalityEvts  */
  else if ((lastEvt_trans_actor_id != trans_actor_id) && (!enableChk)) {
    // else find it in actorMaxEvent and check where it is in the history of
    // last Evt
    immPreEvt = C.findActorMaxEvt(trans_actor_id);
    if (EvtSetTools::contains(C.lastEvent->getHistory(), immPreEvt))
      enableChk = true;
  ...
  // if last evt = preEvt(Isend) or last evt is a Isend, then try to combine
  // with Test
  ...
  if (trans_is_dependent) {
    for (auto it : C.events_) {
      ...
      if ((it_trans_type == "Test") && (it_trans_mb_id == trans_mb_id)) {
        ...
        // tested action is Ireceive, create a new event if it can be marched
        // with the Isend make sure no ancestor candidate event is in history of
        // other ancestor candidate
  ...
  if (check3) {
    ...
    for (auto it : C.events_) {
      ...
      if (trans_dependency && (trans_actor_id != it_trans_actor_id)) {
        // make sure no ancestor candidate event is in history of other ancestor
        // candidate
        EventSet ansestors1;
  ...
  // now combine send and test actions, forming a ancestors set including 3
  // events
  if (lastEvt_trans_actor_id == trans_actor_id) {
...
EventSet createIreceiveEvts(std::string const& trans_tag, Configuration C)
{
  ...
  // if last evt = preEvt(Ireceive) or last evt is a Ireceive, then try to
  // combine with Test
  if (trans_dependency) {
    for (auto it : C.events_) {
      auto it_trans_tag   = it->get_transition_tag();
  ...
  if (lastEvt_trans_actor_id == trans_actor_id) {
    for (auto rEvt : C.events_) {
      ...
      auto check1 = (rEvt_trans_type == "Ireceive") && (rEvt_trans_mb_id == trans_mb_id) &&
                    (rEvt_trans_actor_id != trans_actor_id) && (!EvtSetTools::contains(lastEvtHist, rEvt));
      if (check1) {
        ...
        for (auto tEvt : C.events_) {
          ...
          auto check2 = (tEvt_trans_type == "Test") && (tEvt_trans_actor_id != trans_actor_id) &&
                        (tEvt_trans_mb_id == trans_mb_id);
          if (check2) {
            UnfoldingEvent* testedEvent = C.findTestedComm(tEvt);
            auto testedEvt_trans_tag    = testedEvent->get_transition_tag();
            ...
            auto check3 = (testedEvt_trans_type == "Isend") && (!EvtSetTools::contains(lastEvtHist, tEvt)) &&
...
void UnfoldingChecker::extend(Configuration C, std::list<EventSet> maxEvtHistory, EventSet& exC, EventSet& enC) const
{
  ...
  if (C.events_.empty())
    ...
    // try to create new events from a enabled transition and every maximal_Evt
    // history in maxEvtHistory of C
    for (auto trans_tag : enabled_trans_tags) {
      ...
      auto check_isend = (trans_type == "Isend") &&
                         (trans_dependency || ((lastEvt_trans_type == "Test") && (lastEvt_trans_mb_id == trans_mb_id)));

      auto check_ireceive =
          (trans_type == "Ireceive") &&
          (trans_dependency || ((lastEvt_trans_type == "Test") && (lastEvt_trans_mb_id == trans_mb_id)));

      // if trans is not a wait action, and is dependent with the transition of
      // last event
      if (check_dep_type) {
        EventSet exC1;
        EventSet causalityEvts;
        ...
        // in maxEvent of C, we only consider events having transitions that are
        // dependent with trans
      ...
      // ELSE IF THE TRANSITION IS A WAIT ACTION
        ...
        /* we only call function createWaitEvt if the last action is send/ or
        receive or dependent with the wait (transition in the same actor) */
        auto evt_trans_tag  = evt->get_transition_tag();
        auto evt_trans_type = App::app_side_->get_transition_type(evt_trans_tag);
      ...
      // ELSE IF THE TRANSITION IS A TEST ACTION
        ...
        /* we only call function createTestEvt if the last action is send or
        receive or dependent with the test (transition in the same actor) */
                               ...
                               UnfoldingEvent* currentEvt, EventSet prev_exC)
{
  ...
  // `extend()` method that are contained in C, as
  // ex(C) = { ... e !< C }
  for (auto it : C.events_)
    EvtSetTools::remove(exC, it);

  // return when enC \subset of D
  // If that is the case, than intuitively
  // there aren't any enabled transitions
  // which are "worth" exploring since they're
  // all contained in the sleep set
  //
  // Alternatively, the condition `chk` is already
  // satisfied if enC.size() == 0 since the `if`
  // statement goes unexecuted
  bool chk = true;
  if (enC.size() > 0) {
    for (auto evt : enC) {
  ...
  // INVARIANT: enC.begin() != enc.end() since enC.size() != 0
  if (A.empty())
    e = *(enC.begin());
  else {
  ...
  // update history of the maximal events by adding maximal events of the
  // current Configuration (adding intSet)
  std::list<EventSet> maxEvtHistory1 = maxEvtHistory;
    ...
    if (evt->isImmediateConflict1(evt, e)) {
      ...
      for (auto e2 : h) {
        if (!EvtSetTools::contains(res, e2)) {
          ...
          App::app_side_->delete_state(e2->get_state_id());
        }
      }
    }
  }
