...
bool UnfoldingEvent::check_tr_concern_same_comm(bool& chk1, bool& chk2, UnfoldingEvent* evt1,
                                                UnfoldingEvent* evt2) const
{
...
bool UnfoldingEvent::isImmediateConflict1(UnfoldingEvent* evt1, UnfoldingEvent* evt2) const
{
  ...
  if (EvtSetTools::depends(hist11, hist21) || EvtSetTools::depends(evtS1, hist21) ||
      EvtSetTools::depends(evtS2, hist11)) {
    return false;
  }
...
bool UnfoldingEvent::operator==(const UnfoldingEvent& other) const
{
  ...
  // Causes are the same size
  if (this->causes.size() != other.causes.size())
    return false;

  // Basically, if the causes are the same
  for (auto it : this->causes) {
    bool chk1 = false;
    // Checkes that `it` is contained in the other guy's
    // causes
...
UnfoldingEvent* Configuration::findTestedComm(const UnfoldingEvent* testEvt)
{
  ...
  for (auto it : this->events_) {
    ...
    auto check                  = (it_trans_comm_id == testEvt_trans_comm_id) && (it_trans_type != "Test") &&
